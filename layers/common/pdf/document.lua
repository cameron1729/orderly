...
function Pandoc(document)
  ...
  local function instantiate_definition(blocks, origin, introduce_revision)
    ...
    local instantiated = pandoc.Pandoc(blocks):walk({
      Math = function(math)
        if math.mathtype == 'InlineMath' and math.text:match('^%s*r%s*$') then
          if introduce_revision then
            -- Name the revision once, then use its local alias throughout.
            introduce_revision = false
            return {
              pandoc.Link({code(revision)}, values['revision.url']),
              pandoc.Space(), pandoc.Str('(hereafter'), pandoc.Space(),
              pandoc.Link({code(short)}, values['revision.url']), pandoc.Str(')')
            }
          end
    ...
    assert(not introduce_revision, 'The introduction must identify the revision')
    return instantiated
  end
  ...
  for _, source in ipairs(sources) do
    ...
    if source.sections then
      ...
      rendered = rendered:walk({Para = function(para)
        ...
        return instantiate_definition(source.sections[section], source.definition, section == 'introduction')
      end})
  ...
  if kind == 'correctness' then
    for _, name in ipairs({
      'revision.short', 'verification.url', 'verification.layers.0.url', 'publication.url',
