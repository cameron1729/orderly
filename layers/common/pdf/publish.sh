...
case ${1-} in
  prepare)
    ...
    for path in layers/CORRECTNESS.md layers/0-theory/THEORY.md \
      ...
      layers/0-theory/templates/proof.md layers/common/php/profile.json \
      layers/common/php/expected-opcodes.txt; do
    ...
    jq --slurpfile documents "$work/documents.json" --slurpfile publications "$work/publications.json" \
      --slurpfile runtime "$work/sources/layers/common/php/profile.json" \
      --slurpfile digests "$work/snapshot-digests.json" --arg snapshot "$work/sources" \
      '. + {documents: $documents[0], publications: $publications[0], runtime: $runtime[0],
