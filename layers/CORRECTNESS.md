...
Let $`\mathcal{A}_r`$ denote the public PHP operations at revision $`r`$. For each operation $`f : \mathcal{A}_r`$, let $`\mathcal{G}_r(f)`$ denote its graph: the set of pairs $`(x, y)`$ such that $`f(x)`$ returns $`y`$.

## Layer rules

Let $`\mathcal{L}`$ be [Orderly's layers](../README.md#layers), and $`\mathcal{R}_{\ell,r}`$ the rules declared by layer $`\ell`$ at revision $`r`$. We write $`P(f,p)`$ when the pair $`p \in \mathcal{G}_r(f)`$ satisfies rule $`P`$ for operation $`f`$.
