...
This layer must establish [PHP purity](../PURITY.md) and, using the [shared conformance definition](../CORRECTNESS.md#layer-conformance), $`\mathrm{Correct}_{2}(r)`$.
...
At this revision, the PHP library exposes only `between(): never`. Its [graph](../CORRECTNESS.md#operations-and-results) contains no input–output pairs, so none can violate this layer's rules. This establishes $`\mathrm{Correct}_{2}(r)`$. The empty rule collection would also suffice on its own.
...
[Layer2.lean](lean/Layer2.lean) defines this layer's rules and proves `phpConforms` directly by eliminating an impossible member of the empty function graph in the [shared PHP model](../common/lean/Orderly.lean).

[The verifier](verify) checks the repository premises for both proofs. [The layer action](action.yml) runs that verifier and checks this layer's Lean conformance proof and the shared PHP definitions. Its successful `Layer 2` check supports this layer's badge.
