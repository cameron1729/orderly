...
This layer must establish $`\mathrm{Correct}_{2,{{revision.math}}}`$ under the [shared correctness definition](../../CORRECTNESS.md#layer-correctness), for revision [{{revision.full}}]({{revision.url}}).
...
[The layer check]({{verification.layers.2.url}}) runs the source verifier and checks this layer's Lean input–output proof and the shared PHP definitions. These checks succeeded in [Correct run {{verification.run}}, attempt {{verification.attempt}}]({{verification.url}}) for revision [{{revision.full}}]({{revision.url}}).
