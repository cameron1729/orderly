...
Layer 1 concerns the continuous realization of the [categorical theory](../../0-theory/THEORY.md). The [discrete realization](../../2-discrete-realization/) is derived from this model.

## Claim

This layer must establish $`\mathrm{Correct}_{1,{{revision.math}}}`$ under the [shared correctness definition](../../CORRECTNESS.md#layer-correctness), for revision [{{revision.full}}]({{revision.url}}).

It declares no input–output rules for the PHP library:

$$
\mathcal{R}_{1,{{revision.math}}} = \varnothing.
$$
...
At revision `{{revision.short}}`, the PHP library exposes only `between(): never`. Its [graph](../../CORRECTNESS.md#operations-and-results) contains no input–output pairs, so none can violate this layer's rules. This establishes $`\mathrm{IO}_{1,{{revision.math}}}`$; the empty rule collection would also suffice. Together with purity, it proves $`\mathrm{Correct}_{1,{{revision.math}}}`$.
...
[Layer1.lean](../lean/Layer1.lean) defines this layer's rules and proves `resultsCorrect` directly by eliminating an impossible member of the empty function graph in the [shared PHP model](../../common/lean/Orderly.lean).

[The layer check]({{verification.layers.1.url}}) runs the source verifier and checks this layer's Lean input–output proof and the shared PHP definitions. These checks succeeded in [Correct run {{verification.run}}, attempt {{verification.attempt}}]({{verification.url}}) for revision [{{revision.full}}]({{revision.url}}).
