...
This layer must establish $`\mathrm{Correct}_{1}(r)`$ under the [shared correctness definition](../CORRECTNESS.md#layer-correctness).
...
At this revision, the PHP library exposes only `between(): never`. Its [graph](../CORRECTNESS.md#operations-and-results) contains no input–output pairs, so none can violate this layer's rules. This establishes $`\mathrm{IO}_{1}(r)`$; the empty rule collection would also suffice. Together with purity, it proves $`\mathrm{Correct}_{1}(r)`$.
