...
theorem resultsCorrect {ArrayValue : Type} (rules : List (Rule ArrayValue)) : ResultsCorrect rules := by
...
/-- Apply the theorem to the instructions supplied by the designated opcode check and the
...
theorem correctnessOfVerification
    ...
    (ArrayValue : Type) (programAt : Revision → List Machine.Instruction) (r : Revision)
    ...
    (rules : Document → List ((executionContract ArrayValue programAt).Rule r))
    (sound : VerifierSound programAt recorded designated)
    (verified : Verified recorded w r designated)
    (theoryHash : H (sourceAt r theoryPath) = theoryDigest)
    (definitionHash : H (sourceAt r definitionPath) = definitionDigest) :
    (executionContract ArrayValue programAt).Correct r (rules (sourceAt r theoryPath)) ∧
      H (sourceAt r theoryPath) = theoryDigest ∧
      H (sourceAt r definitionPath) = definitionDigest := by
  obtain ⟨pure, empty⟩ := premisesOfProgram ArrayValue programAt r (sound w r verified)
  exact correctnessOfPremises (executionContract ArrayValue programAt) r sourceAt definitionPath theoryPath H
